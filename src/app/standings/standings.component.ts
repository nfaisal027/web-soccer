...
export class StandingsComponent implements OnInit {
  ...
  constructor(private _titleService:Title, private _soccerService: SoccerService) {
    this._titleService.setTitle('Pertandingan MPL Negara INDONESIA');
    this.MyTeams=[];
    this.MySchedule=[];
    this.Standings=[];
    this.getTeams();
  ...
  public ComputeRangkings(){
    ...
    this.MySchedule.forEach(element => {
      if(element.PlayingDate < curDate && element.HomeScore>=0){
        ...
        this.UpCurrentRow(element,TeamAt,"A")
      }
    })
  }
  private UpCurrentRow(element:Schedule,TeamAt:number,HomeAway:string){
